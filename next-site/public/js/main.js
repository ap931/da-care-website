...
// Basic Mobile Menu / Navbar Toggle Logic
  ...
  if (menuWrapper && toggleBtn) {
    ...
    document.addEventListener('click', (e) => {
        if (menuWrapper.classList.contains('active') && !menuWrapper.contains(e.target)) {
            toggleMenu(false);
        }
    });
  }
...
if (navbar) {
    ...
    const handleScroll = () => {
        // Find the first section or use window innerHeight as a fallback for hero height
        const firstSection = document.querySelector('section');
        const heroHeight = firstSection ? firstSection.offsetHeight : window.innerHeight;
        
        // 30% of the hero height
...
const initPageTransitions = () => {
  ...
  // 2. Check if we should transition "out" (reveal)
  const isTransitioning = sessionStorage.getItem('page-transitioning');
  
  if (isTransitioning === 'true') {
    // Force active state immediately without transition
    overlay.style.transition = 'none';
    overlay.classList.add('is-active');
    
    // Double rAF ensures the browser has rendered the "covering" state 
    // before we start the exit transition, eliminating the flash.
